...
网页接收端在 Worker 中运行 Decimen v0.3 的 ZXing WASM（2 或 3 个 Worker）：优先请求最高 120 FPS（不支持则 60 FPS），用 `requestVideoFrameCallback` 跟随真实取帧。未锁定时单码中心整幅最多 1440 像素；锁定后缩到 960。四码按上一帧码位或重叠象限切成最多 720 像素的小块，每块 `maxSymbols=1`，避免整幅 1600 搜四个码。码已经很大时回到整幅短边，避免近距离裁掉定位点。空结果只再试 GlobalHistogram，不反色。连续 3 次未识别则放开 ROI。忙时丢弃过期帧。界面区分采集、分析和有效二维码 FPS，底部固定“复制诊断”。若页面仍像旧版，强制刷新以更新 Service Worker。旧 `AFL1` 文本流会回退到 jsQR。
