...
- 网页接收端：GitHub Pages HTTPS 页面（当前 **v62**）。单码 Worker 裁 ROI；四码格 4 后只做窗内跟随。细节见 [HANDOVER.md](HANDOVER.md)
...
诊断第一行必须是 `网页：v62`。Chrome 若一直停在旧版，是旧 Service Worker 不刷新；新 SW 激活时会强制打开页面。仍不对就清掉该站数据。
...
| 场景 | 参数 | 说明 |
|---|---|---|
| 单码推荐 | **2331 B · 30 FPS** | QR 约 V34。网页 v61 锁码后约 22 KB/s；v62 缩小锁码后快照，待 Pages 确认 |
| 四码推荐 | **全屏 · 2331 B · 30 FPS** | 发送端会把四码每码限制到 1273 B（QR V22–V25）。对照是 v53 格 4 实时约 50–56 KB/s；v61 重排后掉到约 2 KB/s |
...
网页接收端在 Worker 中运行 Decimen v0.3 的 ZXing WASM（通常 4 个 Worker）。**v62** 单码在主线程做整幅快照、Worker 内裁 ROI；四码锁格走 v53，格 4 后只更新命中落在窗口里的格子。WASM/Worker 走 Service Worker 缓存优先。旧 `AFL1` 文本流会回退到 jsQR。
...
sw.js                                       # 网页接收端缓存（airferry-lite-v62）
