...
网页接收端在 Worker 中运行 Decimen v0.3 的 ZXing WASM（2 或 3 个 Worker）。单码先扫整幅画面最多 1440 像素，锁定后裁紧码再缩到 720–960，避免中心正方形把竖幅上下裁掉。四码先从摄像头抽一次中心 720，再在内存里切 2×2 小块送给 Worker，避免对四个位置分别读视频把采集钉在 12 FPS。锁到 2 格以上才推其余位置。不要把单码切成四块再缩到 720，V34 会不够模块。WASM/Worker 走 Service Worker 缓存优先，避免 `no-store` 卡住解码器。诊断第一行带网页版本号。若标题不是 v31，强制刷新。旧 `AFL1` 文本流会回退到 jsQR。
