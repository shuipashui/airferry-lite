...
网页接收端在 Worker 中运行 Decimen v0.3 的 ZXing WASM（2 或 3 个 Worker）。单码先扫整幅画面最多 1440 像素，锁定后裁紧码再缩到 720–960。四码对齐 APK：Chrome `BarcodeDetector` 只负责找四个框，WASM 按原生分辨率一块一块解二进制（不要先把整幅 2×2 缩到 720 再切，V22 模块会不够）。每帧只从摄像头抽一块，避免采集掉到个位数。不要把单码切成四块再缩到 720。WASM/Worker 走 Service Worker 缓存优先。诊断第一行带网页版本号。若标题不是 v32，强制刷新。旧 `AFL1` 文本流会回退到 jsQR。
