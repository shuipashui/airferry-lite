...
网页接收端在 Worker 中运行 Decimen v0.3 的 ZXing WASM（2 或 3 个 Worker）：优先请求最高 120 FPS（不支持则 60 FPS），用 `requestVideoFrameCallback` 跟随真实取帧。单码把中心区域缩到最多 1440 像素、`maxSymbols=1`；四码最多 1600 像素、`maxSymbols=4`。空结果会依次重试 GlobalHistogram / 反色。命中后跟踪 ROI。忙时丢弃过期帧。界面区分采集、分析和有效二维码 FPS，底部固定“复制诊断”。若页面仍像旧版，强制刷新以更新 Service Worker。旧 `AFL1` 文本流会回退到 jsQR。
