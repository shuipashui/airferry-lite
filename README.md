...
- 网页接收端：GitHub Pages HTTPS 页面（当前 **v53**），支持 Android Chrome 等现代移动浏览器；旧 AFL1 流可 IndexedDB 断点
...
诊断第一行必须是 `网页：v53`。若仍是旧版本，强制刷新或清掉该站数据后再开。
...
| 场景 | 参数 | 说明 |
|---|---|---|
| 单码推荐 | **2331 B · 30 FPS** | QR 约 V34。网页 v53 实测采集约 60、实时约 60 KB/s |
...
网页接收端在 Worker 中运行 Decimen v0.3 的 ZXing WASM（通常 4 个 Worker）。**单码路径已冻结，不要再改**：`ImageBitmap` 扫整幅画面，靠近后不再切中心方块，采集约 60 FPS。四码由 Chrome `BarcodeDetector` 找框，再按四个窗口从一张打包图解码；锁到格 4 后窗口冻结，避免被单个命中拽成格 1。WASM/Worker 走 Service Worker 缓存优先。旧 `AFL1` 文本流会回退到 jsQR。
...
sw.js                                       # 网页接收端缓存（airferry-lite-v53）
