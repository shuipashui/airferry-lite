...
- 网页接收端：GitHub Pages HTTPS 页面（当前 **v58**），支持 Android Chrome 等现代移动浏览器；旧 AFL1 流可 IndexedDB 断点
...
诊断第一行必须是 `网页：v58`。Chrome 若一直停在旧版，是旧 Service Worker 不刷新；新 SW 激活时会强制打开页面。仍不对就清掉该站数据。
...
| 场景 | 参数 | 说明 |
|---|---|---|
| 单码推荐 | **2331 B · 30 FPS** | QR 约 V34。单码始终全图 ImageBitmap、扫描 1440。不要裁 ROI：Android 上裁切 `createImageBitmap` 会把采集从 60 掉到约 12 |
...
网页接收端在 Worker 中运行 Decimen v0.3 的 ZXing WASM（通常 4 个 Worker）。单码走整幅 `ImageBitmap`（不裁切），扫描 1440；四码由 Chrome `BarcodeDetector` 找框，再按四个窗口从一张打包图解码。WASM/Worker 走 Service Worker 缓存优先。旧 `AFL1` 文本流会回退到 jsQR。
...
sw.js                                       # 网页接收端缓存（airferry-lite-v58）
