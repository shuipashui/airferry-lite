...
- 网页接收端：GitHub Pages HTTPS 页面（当前 **v59**），支持 Android Chrome 等现代移动浏览器；旧 AFL1 流可 IndexedDB 断点
...
诊断第一行必须是 `网页：v59`。Chrome 若一直停在旧版，是旧 Service Worker 不刷新；新 SW 激活时会强制打开页面。仍不对就清掉该站数据。
...
| 场景 | 参数 | 说明 |
|---|---|---|
| 单码推荐 | **2331 B · 30 FPS** | QR 约 V34。瞄准时全图 1440；命中后裁到码、扫描 **720**（v26：采集 49、有效 48、实时 69 KB/s）。不要全图 1440 进 WASM，也不要把整幅竖屏压到 720 |
...
网页接收端在 Worker 中运行 Decimen v0.3 的 ZXing WASM（通常 4 个 Worker）。单码先全图瞄准，命中后裁到码并缩到 720；四码由 Chrome `BarcodeDetector` 找框，再按四个窗口从一张打包图解码。WASM/Worker 走 Service Worker 缓存优先。旧 `AFL1` 文本流会回退到 jsQR。
...
sw.js                                       # 网页接收端缓存（airferry-lite-v59）
