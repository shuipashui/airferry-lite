...
网页接收端在 Worker 中运行 Decimen v0.3 的 ZXing WASM（2 个 Worker）：优先请求最高 120 FPS（不支持则 60 FPS），用 `requestVideoFrameCallback` 跟随真实取帧。WASM 输入上限 720 像素：有原生 BarcodeDetector 时先定位再裁小块，否则扫重叠象限或上一帧码位，每块 `maxSymbols=1`。空结果只在连续未识别后重试 GlobalHistogram，不反色。连续 3 次未识别则放开 ROI。诊断第一行带网页版本号；Service Worker 拉取脚本时绕过 HTTP 缓存。若页面标题不是 v23，强制刷新。旧 `AFL1` 文本流会回退到 jsQR。
