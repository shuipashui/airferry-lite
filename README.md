...
- Android 接收端：原生 APK 源码（Android 10+），CameraX 采集与最多 3 路并行 ZXing 解码，支持 `AFL2` 二进制帧并保留旧 `AFL1` 接收回退
- 传输协议：`AFL1` 描述帧、数据帧、GF(256) 线性修复帧、择优 gzip、分片 CRC-32 和原文件 CRC-32
- 高速协议：`AFL2` 二进制帧、LT 喷泉码、V40-L QR、固定掩码 4，默认 30 FPS，并保留 60 FPS 实验选项
...
- 实验：2953 B / 60 FPS（保留给 120 Hz 屏幕或确认发送设备和接收摄像头都能稳定达到 60 FPS 的场景）
- 低密度：1465 B / 24 FPS（距离较远、摩尔纹明显或对焦不稳定时使用）

网页接收端会在 Worker 中运行 Decimen v0.3 使用的 ZXing WASM 解码器：每个摄像头帧只提交给空闲 Worker，忙时丢弃过期帧，不排队堆积；3 个 Worker 与 LT 喷泉码共同吸收丢帧。Android APK 采用同样的调度思路，由 CameraX 提供最新画面，最多 3 个 ZXing 解码线程并行处理，全部繁忙时直接丢弃过期画面。两端都从 QR `BYTE_SEGMENTS` 读取原始二进制帧，在本地完成 LT 解码、gzip 解压和 SHA-256 校验；界面同时显示实时有效载荷速度。
