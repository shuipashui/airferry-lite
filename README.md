...
| 场景 | 参数 | 说明 |
|---|---|---|
| 单码推荐 | **2331 B · 30 FPS** | QR 约 V34。命中后裁码周围（靠近 pad 1.18），不要把整幅 1440×1920 丢给 WASM。v55 全图 1440 会采集约 10 FPS、解码约 75 ms |
...
网页接收端在 Worker 中运行 Decimen v0.3 的 ZXing WASM（通常 4 个 Worker）。单码走 `ImageBitmap`，命中后跟踪码的外接框（靠近用 1.18 倍边距），不要扫整幅竖屏全图。四码由 Chrome `BarcodeDetector` 找框，再按四个窗口从一张打包图解码；锁到格 4 后窗口冻结，避免被单个命中拽成格 1。WASM/Worker 走 Service Worker 缓存优先。旧 `AFL1` 文本流会回退到 jsQR。
