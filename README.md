...
网页接收端在 Worker 中运行 Decimen v0.3 的 ZXing WASM（2 或 3 个 Worker）。单码先扫整幅画面最多 1440 像素，锁定后裁紧码再缩到 720–960，避免中心正方形把竖幅上下裁掉。四码每帧只裁一块（轮转 2×2），攒齐 4 个码位后再跟块，避免同一帧抽 3 张图把采集拖死。不要把单码切成四块再缩到 720，V34 会不够模块。WASM/Worker 走 Service Worker 缓存优先，避免 `no-store` 卡住解码器。诊断第一行带网页版本号。若标题不是 v27，强制刷新。旧 `AFL1` 文本流会回退到 jsQR。
