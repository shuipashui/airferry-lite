...
assert.ok(template.includes('value="dual">双码</option>'), "dual must be a normal layout option");
...
assert.ok(sender.includes("fillHighQueue(dualUpdatesBoth() ? 1 : 2)"));
...
assert.ok(template.includes("双码预填 1465 B · 60 FPS"));
...
assert.ok(sender.includes("dualStaggers()) return Math.max(1, Math.round(vsyncsPerQr / 2))"));
