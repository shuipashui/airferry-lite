...
assert.ok(sender.includes("DUAL_FRAME_BYTES = 1732"));
...
assert.ok(template.includes('value="dual">双码 V30（实验）'), "dual V30 must be an explicit experimental layout");
...
assert.ok(sender.includes("DUAL_FRAME_BYTES) return 30"));
assert.ok(sender.includes("frameBytes === 1273) return 25"));
assert.ok(sender.includes("DUAL_60FPS_MAX_BYTES = 1465"));
assert.ok(sender.includes("fillHighQueue(2)"));
  ...
  assert.ok(sender.includes('if (mode === "dual") return 2'));
  assert.ok(sender.includes("QUAD_MAX_FRAME_BYTES = 1465"));
...
assert.ok(sender.includes("function paintQueued"));
...
assert.ok(template.includes("双码是实验"));
assert.ok(sender.includes("不能替代冻结四码 1465 B · 30 FPS"));
assert.ok(sender.includes("layout === \"dual\""));
assert.ok(sender.includes("双码上排交替更新"));
assert.ok(sender.includes("DUAL_SLOTS = [0, 1]"));
assert.ok(sender.includes("DUAL_SLOTS[highNextPair & 1]"));
assert.ok(sender.includes("codesPerScreen === 4 || codesPerScreen === 2) return Math.max(1, Math.round(vsyncsPerQr / 2))"));
assert.ok(sender.includes("layoutCodes: codesPerScreen === 1 ? 1 : 4"));
assert.ok(template.includes('id="hudPlayBtn"') && template.includes('id="hudFsBtn"'));
assert.ok(template.includes("60 Hz 用 30 FPS"));
