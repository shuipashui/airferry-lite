...
assert.ok(indexHtml.includes("app.js?v=59"), "index.html must cache-bust app.js with the current receiver build");
...
assert.ok(source.includes("if (highScanMisses > 0) return inflateRect(highScanRoi, 1.2 + highScanMisses * 0.2);"), "single-code tracking must crop to the QR instead of decoding the full portrait frame");
...
assert.ok(!source.includes("createImageBitmap(video, {"), "single-code ImageBitmap grabs must pass a source rectangle so Chrome can GPU-scale");
...
assert.ok(serviceWorker.includes('const CACHE_NAME = "airferry-lite-v59";'), "service worker cache version was not bumped");
