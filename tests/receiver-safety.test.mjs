...
assert.ok(indexHtml.includes("app.js?v=78"), "index.html must cache-bust app.js with the current receiver build");
...
assert.ok(!source.includes("highMultiLayout && locked < 4"), "quad must not keep BarcodeDetector.detect(video) running after the grid is known");
...
assert.ok(serviceWorker.includes('const CACHE_NAME = "airferry-lite-v78";'), "service worker cache version was not bumped");
