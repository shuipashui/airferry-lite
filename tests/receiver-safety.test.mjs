...
assert.ok(indexHtml.includes("app.js?v=58"), "index.html must cache-bust app.js with the current receiver build");
...
assert.ok(source.includes("const bitmap = (!tile && !highMultiLayout)"), "single-code ImageBitmap grabs must not crop the video on Android");
assert.ok(!source.includes("if (!highMultiLayout && !highScanRoi) return;"), "single-code speed must not wait for an ROI crop that is no longer used");
...
assert.ok(serviceWorker.includes('const CACHE_NAME = "airferry-lite-v58";'), "service worker cache version was not bumped");
