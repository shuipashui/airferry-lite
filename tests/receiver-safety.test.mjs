...
for (const needle of [
  ...
  ,"HIGH_SINGLE_INFLIGHT = /Android/i.test(navigator.userAgent || \"\") ? 1 : 4;"
...
assert.ok(indexHtml.includes("app.js?v=82"), "index.html must cache-bust app.js with the current receiver build");
...
assert.ok(source.includes("lastSingleGrabAt && now - lastSingleGrabAt < HIGH_SINGLE_GRAB_MS"), "single-code grabs must wait ~33 ms so a 60 FPS camera does not sample the 30 FPS sender between frames");
...
assert.ok(serviceWorker.includes('const CACHE_NAME = "airferry-lite-v82";'), "service worker cache version was not bumped");
