...
**交接时点：** 2026-08-21。网页接收端 **v85**。Android APK **0.8.17**。发送端打开双码预填 **1465 B · 60 FPS**（两格同时更新）。红米窗口收完会话 **168.6 KB/s**，贴上 V27 双码上限约 169。四码 1465·30 窗口对照仍是 **152.8 KB/s**。发送端是根目录 `sender/` 打出的单文件 HTML。以 Pages 上的 `sender/dist/airferry-lite-sender.html` 为准。
...
| Android APK | **0.8.17**（versionCode 32） | 红米窗口 · 相机 60。双码 **1465 B · 60 FPS** 收完会话 **168.6 KB/s**（每帧 1.90）。四码 1465·30 窗口 **152.8** / 拿远 60 **155.3**。0.8.16 起清空不重绑相机；0.8.17 两格命中重排格子 |
| 发送端 | AFL2 单文件 HTML | 打开单码预填 **2953 B · 30 FPS**；打开四码预填 **1465 B · 30 FPS**（60 Hz）；打开双码预填 **1465 B · 60 FPS**。四码每码上限 **1465 B**。无 45 FPS。整数倍模块；四码交错换对角；双码 60 FPS 两格同时更新 |
...
**当前最快：1465 B · 双码 · 60 FPS · 相机 60**（V27，k=969，两格同时更新）。载荷上限 `1445 × 2 × 60 / 1024 ≈ 169 KB/s`。

| 发送 | 距离 | 采集 / 分析 | 每帧 | ROI | 唯一 / 重复 | 实时 / 平均 / 会话 |
|---|---|---|---|---|---|---|
| **1465 B · 60 FPS · 双码** | 窗口 | 59.9 / 59.9 · 10.2 ms | **1.90** | 格 2 | 1687 / 191 | 161.9 / 166.5 / **168.6 KB/s** |

不要交错双码 60 FPS：一帧只换一格时唯一大约减半，会话会掉到约 **84 KB/s**。30 FPS 双码才交错。1732 B（V30）不要跑 60 FPS，生成跟不上。

### 四码对照（0.8.14 路径，0.8.17 仍成立）

**1465 B · 四码 · 相机 60 · 非全屏**。构图比帧率更关键：四个码都要进画面。
...
- 四码发送 30 默认更稳（每帧 2.68）。拿远 60 FPS 会话 **155.3**、实时 **170**，贴上四码 V27 上限 `1445 × 4 × 30 / 1024 ≈ 169`（60 Hz 交错的唯一码仍约 120/秒，不是 240）。
- 60 FPS 拿近掉到 125.6，是取景框装不下 2×2、重复涨到 1187，不是「60 这个数字本身一定慢」。
- 交错时格 2 / 格 3 常见。
...
- 动手前红米 A/B：双码 1465·60 不能低于会话 **168.6 KB/s**；四码不能低于 **152.8**（拿远 60 对照 **155.3**）。
...
2. `LTEncoder` 按 `每帧数据 − 20` 切块。四码 `QUAD_MAX_FRAME_BYTES = 1465`（V27）。双码 `DUAL_FRAME_BYTES = 1732`（V30），60 FPS 上限 `DUAL_60FPS_MAX_BYTES = 1465`（V27）。每帧下拉：四码 1003 / 1273 / 1465，单码 1465 / 2331 / 2953，双码 1003 / 1273 / 1465 / 1732。帧率下拉：单码 20 / 24 / 30；四码另加 60 / 90 / 120（高刷）；双码 20 / 24 / 30 / 60（60 Hz 预填 **1465 B · 60 FPS**）。选 60 FPS 时双码自动改 1465 B。**不要 45 FPS**。`qr.make(4)` 必须钉死版本号（1465→27、1273→25、1732→30、2953→40），不要 `qrcode(0)` 从 1 扫到 N。双码 **`layoutCodes = 4`**，只占 2×2 上排。下排复制已实测否定。**不要把双码设成打开页面的默认布局**（默认仍是单码）。
...
7. **双码：** **60 FPS 必须两格同时更新**，否则 60 Hz 上唯一码只有 60/秒（约 84 KB/s）。30 FPS 才交错一格。APK 0.8.17 两格命中会重排格子。1465·60 窗口收完：每帧 **1.90**，唯一 1687 / 重复 191，实时 161.9，会话 **168.6 KB/s**。1732·60 生成跟不上。
...
| 双码 | **1465 B · 60 FPS**（60 Hz 预填；只占上排两格。30 FPS 可用 1732 B） |
...
来源：2026-08-21 Cursor 画布 `apk-sender-speed-levers`（未进 git）。已落地：整数放大、交错换对角、1–2 命中跟格、锁满后跳过补扫、四码 **1465 B（V27）**、双码 **1465 B · 60 FPS**（会话 168.6）。下面不要当回归清单自动执行；动手前红米 A/B，双码 1465·60 不能低于 **168.6 KB/s**，四码不能低于 **152.8**（拿远 60 对照 **155.3**）。

不要做（画布写过、实测否定）：四码分析改 30；60 Hz 四码 60 FPS **拿太近**（125.6）；**60 Hz 双码 1732·60**（生成跟不上）；**60 Hz 双码 60 FPS 交错一格**（唯一减半，约 84 KB/s）。
...
| P2 | 发送端 | 双码 1465·60 | **已落地。** APK 0.8.17 窗口收完：每帧 1.90，唯一 1687 / 重复 191，会话 **168.6 KB/s**（上限约 169）。打开双码预填 1465·60，两格同时更新。1732 仍只给 30 FPS |
