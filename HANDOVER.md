...
- bind 后**只有继续接收冷启动**才点测上中 AE/AF 并在空帧轻推曝光补偿（0.8.56 把点测加到每一次 bind → 首次 97.2 / 每帧 0.11）。首次接收不点测、不轻推。收完 `stopScanner` 不清曝光计数。不要为此 `unbindAll`，不要默认锁死 AE/AWB。不要空扫 `killProcess`。
...
- 不要一锁 格 2 就跳过四格补扫（0.8.36：四码约 84）。不要凭单枚 **四码** `layoutCodes=4` 锁多码。不要凭单枚 `layoutCodes=2` 锁双码并跳过补扫（0.8.50）。不要在见过一枚双码之后对空帧串行补扫（0.8.52：空白后首次 5 KB/s）。不要未锁时因 1 枚 `0x1c` 串行追兄妹（0.8.53：空白后首次 38 KB/s）。不要空扫 3 秒就 `killProcess`。不要下排复制 QR / 第三枚标记码 / 深色块（0.8.50 已否）。不要为了对着码第一次 bind 去改 0.8.54 已能满速的裁切；过一会重开 240 说明是 HAL/AE。不要在每一次首次 bind 上点测 AE（0.8.56：首次 97.2 / 每帧 0.11）。点测 AE 必须 2 秒自动取消，不要默认锁死 AE/AWB。
