...
**交接时点：** 2026-08-22。网页接收端 **v86**。Android APK **0.8.62**。给蓝只发 Artifacts 直链。**0.8.43 冻结：** 首次 234.6、继续 238.4、强杀 236.2。**0.8.44–0.8.46 已否。** 发送端双码仍是 2×2 上排，帧头 **`layoutCodes=2`**，下排白底。**0.8.52–0.8.53 已否。** **0.8.54：** 未锁只便宜 max4 + 半幅。**0.8.55：** 多次首次 240。**0.8.56：** 首次 97.2。**0.8.57：** 首次 148.1；继续 144.6 / 点测 11。**0.8.59：** 首次和继续可用；杀后台后再收 0 命中（点测 1）。**0.8.60：** 去掉点测/轻推。脏退出等 2 秒再 bind；继续接收不要闪「接收文件」；清空只换解码不重绑；单图预览恢复。**0.8.61：** 双码空扫 2 帧就清 dualHint 导致锁单码；清空 recoverPipeline + 1.2s settle；四码重叠象限。**0.8.62：** 手动「接收文件」不再因陈旧 `camera_held` 多等 2 秒；冷启动清 held，bind 后 1.2s 再挂分析。继续接收仍 2 秒倒计时冷启动。不要 onStop unbind。不要看门狗 unbind。
...
| Android APK | **0.8.62**（versionCode 77） | 手动「接收文件」不再多等 2 秒；0.8.61 双码/清空/四码修复 |
