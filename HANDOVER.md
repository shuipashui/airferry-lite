...
**交接时点：** 2026-08-22。网页接收端 **v85**。Android APK **0.8.26**。0.8.25 再扫仍不稳（98 / 238.6 / 120）。等一会再开能满速，接着再扫又掉一半（热衰减，不是杀进程才好）。当前最快：双码 **2068 B · 60 FPS** 会话 **240.0 / 238.6 KB/s**。发送端打开双码预填 2068·60。
...
| Android APK | **0.8.26**（versionCode 41） | 收完后停分析（预览仍开、不重绑相机）。清空重建 zxing，不 `unbindAll`。至少 2 命中才进多码布局。格子 <2 只整幅 max4，不再叠四格裁切 |
...
双码只扫到一枚时会话约为上限的一半（2068 约 **98–120**，1732 约 **84**）。0.8.25 再扫仍会腰斩：一枚 dual 帧就把 `multiLayout` 打开，收完后分析还在 60 FPS 空转，久开会热到扫不动。**0.8.26：至少 2 命中才进多码；格子 <2 只整幅 max4；收完停分析；清空重建 zxing、不重绑相机。** 不要交错双码 60 FPS。不要 60 Hz 四码四格同刷。
...
- 动手前红米 A/B：双码 2068·60 **首次/冷却后**不能低于 **238.6 KB/s**；再扫也不应腰斩到约 98–120。等一会再开能满速、接着又一半，算同一条热衰减，不要当新机型。1952·60 不能低于 **219.9**；1732·60 不能低于 **190.4**。
...
## 7. Android APK 实现（0.8.26）

源码：`android-receiver/app/src/main/java/com/airferrylite/receiver/`。构建：`android-receiver/build-local.ps1` 或 GitHub Actions `Build Android receiver`。Java 17，SDK 35。`versionName 0.8.26` / `versionCode 41`。不要改解码选项（`tryHarder` / rotate / invert / downscale / `isPure`），除非明确要求动分析管线。
...
- 长时间开着会卡：解码超过 400 ms 或相机时间戳不涨或诊断心跳 >2 s，看门狗才重绑 CameraX 并重建 zxing 线程。**收完后停分析**（预览仍开）。**「清空重收」重建 zxing，不重绑相机**。不要为了省电自动关分析流。
...
android-receiver/               Kotlin + CameraX + zxing-cpp（0.8.26）
...
| P2 | 发送端 | 双码 2068·60 | **已落地（首次）。** 会话 **240.0 / 238.6**。0.8.25 再扫 98 / 120。0.8.26 收完停分析、2 命中才 multiLayout |
