...
**交接时点：** 2026-08-22。网页接收端 **v85**。Android APK **0.8.25**（0.8.24 清空后若先打中 1 枚会卡在半速）。当前最快：双码 **2068 B · 60 FPS** 首次会话 **240.0 KB/s**（实时 238.0，贴 V33 上限 240）。1952·60 对照 **219.9**。四码 **1465 B · 30 FPS 整屏同换** 会话 **168.9**。发送端打开双码预填 2068·60。以 Pages 上的 `sender/dist/airferry-lite-sender.html` 为准。
...
| Android APK | **0.8.25**（versionCode 40） | 红米窗口 · 相机 60。**清空仍不重绑相机**。格子 <2 时整幅 `maxSymbols=4` 重新锁双码；已锁 2 格且两枚都打中则不要整幅 max4（0.8.19）。双码 2 格空扫 6 帧才丢格 |
...
|---|---|---|---|---|---|---|
| **2068 B · 60 FPS · 双码同刷** | 窗口 · **首次** | 60.2 / 60.2 · 11.8 ms | **1.94** | 格 2 | 1375 / 121 | 238.0 / 239.8 / **240.0 KB/s** |
| 2068 B · 60 FPS · 双码同刷 | 窗口 · 0.8.24 再扫 | 58.7 / 58.7 · 11.7 ms | **0.96** | 收完全图 | 1219 / 87 | 119.7 / 119.7 / **114.7 KB/s** |
...
双码只扫到一枚时会话约为上限的一半（2068 约 **114**，1732 约 **84**；每帧 ~0.9、实时也腰斩）。0.8.24 再扫 2068 就是这条：清空后若先打中 1 枚，`multiLayout` 已开、格子仍 <2，之后只按四格 `maxSymbols=1` 裁，再也找不到第二枚。**0.8.25：格子 <2 时整幅 `maxSymbols=4` 重新锁；已锁 2 格且两枚都打中不要整幅 max4。清空仍只 `resetSession`，不要 `unbindAll`。** 不要交错双码 60 FPS（约 84 KB/s）。不要 60 Hz 四码四格同刷。
...
- 动手前红米 A/B：双码 2068·60 **首次**不能低于 **240.0 KB/s**；再扫也不应腰斩到约 114。1952·60 不能低于 **219.9**；1732·60 不能低于 **190.4**；四码 1465·30 整屏同换不能低于 **168.9**；双码 1465·60 对照 **168.6**。
...
## 7. Android APK 实现（0.8.25）

源码：`android-receiver/app/src/main/java/com/airferrylite/receiver/`。构建：`android-receiver/build-local.ps1` 或 GitHub Actions `Build Android receiver`。Java 17，SDK 35。`versionName 0.8.25` / `versionCode 40`。不要改解码选项（`tryHarder` / rotate / invert / downscale / `isPure`），除非明确要求动分析管线。
...
- 四码：已有格子则 4 路并行；锁满且本帧 ≥3 命中则返回，不再串行补扫。≥3 命中才 `tilesFromHits`。四码 1–2 命中只 `followContainedHits`。**双码首次 ≥2 命中才锁格；已锁 2 格后保持格子，不要每帧重排。** 已锁 ≥2 格却只命中 1 枚：整幅补扫；连续 3 帧不够则丢格。**格子 <2 时也要整幅 `maxSymbols=4` 重新锁**（0.8.24 再扫半速：1 枚就把 multiLayout 打开，之后只裁四格 max1）。双码 1 命中不把 ROI 收到那一格。已锁 2 格且两枚都打中时不要整幅 max4（0.8.19）。空扫：无锁 2 次清；已锁 ≥2 格要 6 次。
...
android-receiver/               Kotlin + CameraX + zxing-cpp（0.8.25）
...
- 不要为杀后台 / resume 再出 APK。resume 重建 zxing 已否。0.8.25 只修双码格子 <2 时的整幅重锁，不重绑相机。
...
| P2 | 发送端 | 双码 2068·60 | **已落地（首次）。** 0.8.24 窗口首次：每帧 1.94，实时 238.0，会话 **240.0 KB/s**。再扫 114.7 / 每帧 0.96 是 1 枚陷阱，0.8.25 修 |
