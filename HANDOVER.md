...
**交接时点：** 2026-08-21。网页接收端 **v85**。Android APK **0.8.14** + 发送端四码 **1465 B**。四格都进画面时窗口会话 **152.8 KB/s（30 FPS）** / **155.3 KB/s（60 FPS 拿远）**。发送端是根目录 `sender/` 打出的单文件 HTML。以 Pages 上的 `sender/dist/airferry-lite-sender.html` 为准。
...
| Android APK | **0.8.14**（versionCode 29） | 红米窗口 · **1465 B** · 四码 · 相机 60。四格都进画面时：30 FPS 会话 **152.8 KB/s**，60 FPS 拿远会话 **155.3 KB/s**（实时约 170，贴 V27 唯一载荷上限 169） |
...
来源：2026-08-21 Cursor 画布 `apk-sender-speed-levers`（未进 git）。已落地：整数放大、交错换对角、1–2 命中跟格、锁满后跳过补扫、四码 **1465 B（V27）**。下面不要当回归清单自动执行；动手前红米 A/B，不能低于会话 **152.8 KB/s**（拿远 60 FPS 对照 **155.3**）。

不要做（画布写过、实测否定）：四码分析改 30；60 Hz 四码 60 FPS **拿太近**（125.6）。拿远四格都进框时 60 FPS 可以到 155.3，与 30 FPS 的 152.8 同一档。
