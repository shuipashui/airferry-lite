...
**交接时点：** 2026-08-22。网页接收端 **v85**。Android APK **0.8.47**。给蓝只发 Artifacts 直链。**0.8.43 冻结：** 首次 234.6、继续 238.4、强杀 236.2。**0.8.44 已否：** 首次 25.9 FPS / 0.27 / 29.7。**0.8.45 已否：** 0 命中。**0.8.46 已否：** 对角双码 43 FPS / 0.88 / 95.5。**0.8.47：** 发送改回 2×2 上排；解码回到 0.8.43（max4 + 左右两裁 + 格 2 再扫左右；不要跳过串行补扫）。空扫 3 秒冷启动仍保留。不要 onStop unbind。不要看门狗 unbind。
...
| Android APK | **0.8.47**（versionCode 62） | 上排双码；解码回到 0.8.43。对角已否。0.8.43 首次/继续/强杀满速仍要保住 |
...
7. **双码：** **60 FPS 必须两格同时更新**。发送画在 2×2 **上排**。不要 60 FPS 交错只换一格。不要对角（0.8.46：格 2 但每帧 0.88、会话 95.5）。诊断「布局 四码」是 `layoutCodes=4`，不是扫到四格。2068·60 对照仍以 0.8.43 首次 **234.6** / 继续 **238.4** / 强杀 **236.2** 为准。
...
## 7. Android APK 实现（0.8.47）

源码：`android-receiver/app/src/main/java/com/airferrylite/receiver/`。构建：`android-receiver/build-local.ps1` 或 GitHub Actions `Build Android receiver`。Java 17，SDK 35。`versionName 0.8.47` / `versionCode 62`。不要改解码选项（`tryHarder` / rotate / invert / downscale / `isPure`），除非明确要求动分析管线。
...
- 长时间开着会卡：解码超过 400 ms 只换 zxing 对象，不要停分析。看门狗心跳死了或解码超时**只 `replaceDecoders`，不要 `unbindAll`**（0.8.42 清空看门狗 1 次，会话 151、实时 240）。**进应用不开相机**。这台小米**每个进程只能成功 bind 一次**。收完 `unbindAll`。「继续接收」先停相机等 2 秒再 `killProcess`。不要 `onStop` unbind。不要 `shutdownAsync`。扫描中清空 `resetSession` 清格子和 ROI，不 `unbindAll`（0.8.43 清空保格后 miss 掉格剩跟踪中，26 FPS / 1.9）。连续 miss 清格时也清 ROI。不要首帧预热。不要独立 Lifecycle。不要把格 2 提前 return。诊断始终完整文本，高度 48dp。约 3 秒全速空扫且 0 命中则冷启动一次；20 秒内不循环。**0.8.46 对角已否：** 43 FPS / 18 ms / 每帧 0.88 / 会话 95.5。
...
android-receiver/               Kotlin + CameraX + zxing-cpp（0.8.47）
...
- 不要把格 2 当双码提前返回、跳过四格补扫（0.8.36：四码约 84 KB/s）。不要用 1 枚命中 `pairFromHit` 猜邻格。不要用左右两裁**替换**未锁格 max4（0.8.33：双码 0 命中）；max4 之后不足 2 枚再左右两裁可以。不要把双码改成对角（0.8.46：95.5）。不要在已锁 2 格且两枚都打中时整幅 max4（0.8.19）。
