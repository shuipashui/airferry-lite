...
**交接时点：** 2026-08-22。网页接收端 **v85**。Android APK **0.8.31**。点「接收文件」才开相机；收完 `unbind` 进预览。诊断常显 48dp。扫描中清空只 `resetSession`。**0.8.30 已否：** `shutdownAsync` + 收完不拆会话把第一次绑成 ~22 KB/s 慢会话还一直占着，后面也满不了。不要再 shutdown / 独立 Lifecycle / 首帧预热。当前最快：双码 **2068 B · 60 FPS** 首次 **240.0 / 233.9**。
...
| Android APK | **0.8.31**（versionCode 46） | 点「接收文件」才开相机；收完 `unbind` 进预览。诊断常显 48dp。扫描中清空不重绑。看门狗仅心跳死了才重绑。0.8.30 shutdown/保会话已否 |
...
## 7. Android APK 实现（0.8.31）

源码：`android-receiver/app/src/main/java/com/airferrylite/receiver/`。构建：`android-receiver/build-local.ps1` 或 GitHub Actions `Build Android receiver`。Java 17，SDK 35。`versionName 0.8.31` / `versionCode 46`。不要改解码选项（`tryHarder` / rotate / invert / downscale / `isPure`），除非明确要求动分析管线。
...
- 长时间开着会卡：解码超过 400 ms 只换 zxing 对象，不要停分析。相机时间戳不涨或诊断心跳 >2 s，看门狗才重绑 CameraX。**进应用不开相机**；点「接收文件」才 `bindToLifecycle(Activity)`。PreviewView 空闲时 `invisible`，`FILL_CENTER`。**收完 `unbindAll` 进预览**；「继续接收」再绑。扫描中清空只 `resetSession`。不要 `shutdownAsync`、不要独立 Lifecycle 挡住 `onStop`、不要首帧 `unbind` 预热。诊断始终完整文本，高度 48dp。
...
android-receiver/               Kotlin + CameraX + zxing-cpp（0.8.31）
...
- 不要为清空或半速去 `unbindAll` / 重绑 CameraX（红米上一次半速、两次没画面）。看门狗只在画面真卡住时重绑。收完关相机、点「继续接收」再开（0.8.28 这条路上第二次能到 240）。不要 `shutdownAsync` 预热（0.8.30：首次 ~22 KB/s 且后面也满不了）。不要首帧 `unbind` 预热（0.8.29：预热完成仍 0 命中）。不要独立 Lifecycle 挡住 Activity `onStop`。
