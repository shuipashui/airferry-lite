...
**交接时点：** 2026-08-22。网页接收端 **v85**。Android APK **0.8.27**。0.8.26 连扫五次（每次清空）：233.9 / 45.4 / 226.2 / 24.4 / 0，看门狗恢复 0→5。第一次收完停分析是对的（诊断里分析 1.0 FPS）；清空杀线程池触发了 CameraX 重绑。0.8.27 清空只 `resetSession`。当前最快：双码 **2068 B · 60 FPS** 首次 **240.0 / 233.9**。
...
| Android APK | **0.8.27**（versionCode 42） | 收完后停分析。清空只复位会话，不杀解码线程、不 `unbindAll`。解码超时只换 zxing 对象。看门狗仅心跳死了才重绑相机 |
...
## 7. Android APK 实现（0.8.27）

源码：`android-receiver/app/src/main/java/com/airferrylite/receiver/`。构建：`android-receiver/build-local.ps1` 或 GitHub Actions `Build Android receiver`。Java 17，SDK 35。`versionName 0.8.27` / `versionCode 42`。不要改解码选项（`tryHarder` / rotate / invert / downscale / `isPure`），除非明确要求动分析管线。
...
- 长时间开着会卡：解码超过 400 ms 只换 zxing 对象，不要停分析。相机时间戳不涨或诊断心跳 >2 s，看门狗才重绑 CameraX。**收完后停分析**（预览仍开；诊断里分析约 1 FPS 是正常的）。**「清空重收」只 `resetSession`，不杀线程、不重绑相机。**
...
android-receiver/               Kotlin + CameraX + zxing-cpp（0.8.27）
