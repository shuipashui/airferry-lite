...
**交接时点：** 2026-08-22。网页接收端 **v85**。Android APK **0.8.28**。进应用不开相机；点「接收文件」才绑 CameraX；收完 `unbind` 进预览（保存 / 继续接收）。诊断常显、高度 48dp。清空扫描中仍只 `resetSession`。0.8.26 连扫五次清空会把相机打残。当前最快：双码 **2068 B · 60 FPS** 首次 **240.0 / 233.9**。
...
| Android APK | **0.8.28**（versionCode 43） | 点「接收文件」才开相机；收完关相机进预览。诊断常显 48dp。扫描中清空不重绑。看门狗仅心跳死了才重绑 |
...
## 7. Android APK 实现（0.8.28）

源码：`android-receiver/app/src/main/java/com/airferrylite/receiver/`。构建：`android-receiver/build-local.ps1` 或 GitHub Actions `Build Android receiver`。Java 17，SDK 35。`versionName 0.8.28` / `versionCode 43`。不要改解码选项（`tryHarder` / rotate / invert / downscale / `isPure`），除非明确要求动分析管线。
...
- 长时间开着会卡：解码超过 400 ms 只换 zxing 对象，不要停分析。相机时间戳不涨或诊断心跳 >2 s，看门狗才重绑 CameraX。**进应用不开相机**；点「接收文件」才 `bindToLifecycle`。**收完 `unbindAll`，进文件预览**（图片直接显示）。「继续接收」再绑一次相机。扫描中「清空重收」只 `resetSession`，不重绑。诊断始终完整文本，高度 48dp，不要点进度条才展开。
...
android-receiver/               Kotlin + CameraX + zxing-cpp（0.8.28）
...
- 不要为清空或半速去 `unbindAll` / 重绑 CameraX（红米上一次半速、两次没画面）。看门狗只在画面真卡住时重绑。收完关相机、点「继续接收」再开是允许的。
