...
0.8.17 双码久播可能掉到约 **84 KB/s**（每帧 0.87、重复 0）。0.8.21–0.8.23 在清空 / 回前台 / 启动时重绑 CameraX：红米上点一次清空掉到一半，再点一次没速度。这是 0.8.16 已经否定的路径。0.8.24：**清空只 `resetSession`，不 `unbindAll`**；看门狗仅在画面真卡住时重绑。清空后若第一帧只命中 1 枚，不再把 ROI 收到那一格（否则四分扫描永远看不到另一枚）。双码锁 2 格后保持格子大小，不每帧 `tilesFromHits` 收缩。**不要再做 0.8.19**。双码必须走两格 `maxSymbols=1` 裁剪。不要为了半速恢复去重绑相机。
