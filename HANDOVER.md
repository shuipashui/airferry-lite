...
**交接时点：** 2026-08-22。网页接收端 **v86**。Android APK **0.8.63**。给蓝只发 Artifacts 直链。**0.8.43 冻结：** 首次 234.6、继续 238.4、强杀 236.2。**0.8.44–0.8.46 已否。** 发送端双码仍是 2×2 上排，帧头 **`layoutCodes=2`**，下排白底。**0.8.52–0.8.53 已否。** **0.8.54：** 未锁只便宜 max4 + 半幅。**0.8.55：** 多次首次 240。**0.8.56：** 首次 97.2。**0.8.57：** 首次 148.1；继续 144.6 / 点测 11。**0.8.59：** 首次和继续可用；杀后台后再收 0 命中（点测 1）。**0.8.60：** 去掉点测/轻推。脏退出等 2 秒再 bind；继续接收不要闪「接收文件」；清空只换解码不重绑；单图预览恢复。**0.8.61：** 双码空扫 2 帧就清 dualHint 导致锁单码；清空 recoverPipeline + 1.2s settle；四码重叠象限。**0.8.62：** 手动「接收文件」不再因陈旧 `camera_held` 多等 2 秒；冷启动清 held，bind 后 1.2s 再挂分析。**0.8.63：** 关 app 立刻重开 98% 空扫 → 按 `camera_release_ms` 只补等未满 2 秒；>80% 空扫自动冷启动；协议帧维持 dual/quad hint。不要 onStop unbind。不要看门狗 unbind。
...
| Android APK | **0.8.63**（versionCode 78） | HAL 冷却时间戳；空扫自动冷启动；协议维持布局 hint |
...
|---|---|---|---|---|---|---|
| 2068 B · 60 FPS · 双码同刷 | 窗口 · 0.8.59 杀后台后再收 | 60.0 / 60.0 · 8.4 ms | **0.00** · 点测 1 | 全图 · 单码 | 0 / 0 | 会话 **0 B/s** |
...
## 7. Android APK 实现（0.8.60）

源码：`android-receiver/app/src/main/java/com/airferrylite/receiver/`。构建：`android-receiver/build-local.ps1` 或 GitHub Actions `Build Android receiver`。Java 17，SDK 35。`versionName 0.8.60` / `versionCode 75`。不要改解码选项（`tryHarder` / rotate / invert / downscale / `isPure`），除非明确要求动分析管线。
...
- bind 后**不要点测、不要轻推 EV**。0.8.43 满速没有曝光补偿。0.8.55–0.8.59 的点测/轻推造成继续闪烁和速度不稳（0.8.57 继续点测 11；0.8.59 杀后台点测 1 仍 0 命中）。重叠裁切里一枚命中只归最近的那一格，另一格继续补扫。不要为此 `unbindAll`，不要默认锁死 AE/AWB。不要空扫 `killProcess`。
...
- 长时间开着会卡：解码超过 400 ms 只换 zxing 对象，不要停分析。看门狗心跳死了或解码超时**只 `replaceDecoders`，不要 `unbindAll`**。**进应用不开相机**。这台小米**每个进程只能成功 bind 一次**。收完 `unbindAll`。「继续接收」按下后再等 **2 秒**冷启动（倒计时等待页，不要立刻 `killProcess`）。新进程不要闪「接收文件」闲置页，直接「正在打开相机」，再等 2 秒 bind、预览 1.2 秒再挂分析。手动「接收文件」**不要**再因陈旧 `camera_held` 多等 2 秒（0.8.62）；冷启动清 held，bind 后 1.2 秒再挂分析。杀后台后再收若仍 0 命中再单独加等待，不要拖慢正常首次开相机。不要 `onStop` unbind。不要 `shutdownAsync`。扫描中清空 `recoverPipeline` + 1.2s settle，不要重绑。不要空扫自动冷启动。不要首帧预热。不要独立 Lifecycle。诊断始终完整文本，高度 48dp。
- 单图恢复预览；多文件 zip 只列文件名。点「保存文件」，不要自动写盘。诊断 ROI 显示 `格 N`。进度只在内存。
...
android-receiver/               Kotlin + CameraX + zxing-cpp（0.8.60）
...
- 不要一锁 格 2 就跳过四格补扫（0.8.36：四码约 84）。不要凭单枚 **四码** `layoutCodes=4` 锁多码。不要凭单枚 `layoutCodes=2` 锁双码并跳过补扫（0.8.50）。不要在见过一枚双码之后对空帧串行补扫（0.8.52：空白后首次 5 KB/s）。不要未锁时因 1 枚 `0x1c` 串行追兄妹（0.8.53：空白后首次 38 KB/s）。不要空扫 3 秒就 `killProcess`。不要下排复制 QR / 第三枚标记码 / 深色块（0.8.50 已否）。不要为了对着码第一次 bind 去改 0.8.54 已能满速的裁切；过一会重开 240 说明是 HAL/AE。不要为了扫双码去点测或轻推曝光（0.8.55–0.8.59：继续闪烁、速度不稳）。重叠格不要把一枚命中算成两格都已覆盖。不要默认锁死 AE/AWB。
