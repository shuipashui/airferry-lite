...
**交接时点：** 2026-08-22。网页接收端 **v85**。Android APK **0.8.17**。60 Hz 上 V27 唯一上限约 **169 KB/s**。双码 **1465 B · 60 FPS** 会话 **168.6**；四码 **1465 B · 30 FPS 整屏同换** 会话 **168.9**。发送端打开双码预填 1465·60，打开四码预填 1465·30。以 Pages 上的 `sender/dist/airferry-lite-sender.html` 为准。
...
| Android APK | **0.8.17**（versionCode 32） | 红米窗口 · 相机 60。双码 **1465·60** 会话 **168.6**（每帧 1.90）。四码 **1465·30 整屏同换** 会话 **168.9**（每帧 2.94）。四码 60 交错约 **154**。0.8.16 起清空不重绑相机 |
| 发送端 | AFL2 单文件 HTML | 打开单码预填 **2953 B · 30 FPS**；打开四码预填 **1465 B · 30 FPS**（整屏同换）；打开双码预填 **1465 B · 60 FPS**。60 FPS 四码仍交错。无 45 FPS |
...
60 Hz 上 V27 唯一载荷上限约 **169 KB/s**（`1445 × 120 唯一码/秒 / 1024`）。双码 60 两格同刷、四码 30 整屏同换，都是 120 唯一码/秒。
...
四码 30 的重复 2421 来自相机 ~60 FPS 扫 30 FPS 画面（同一屏会扫两遍），会话仍贴上限。不要因此改回交错：旧交错 30 会话只有 **152.8**（每帧 2.68，唯一 1188 / 重复 784）。

不要交错双码 60 FPS（约 84 KB/s）。不要 60 Hz 四码四格同刷。1732 B 不要跑 60 FPS。
...
- 动手前红米 A/B：四码 1465·30 整屏同换不能低于 **168.9 KB/s**；双码 1465·60 不能低于 **168.6**。四码 60 交错不必追，对照约 154。
...
7. **双码：** **60 FPS 必须两格同时更新**。窗口收完会话 **168.6 KB/s**。30 FPS 才交错一格。1732·60 生成跟不上。
...
来源：2026-08-21 Cursor 画布 `apk-sender-speed-levers`（未进 git）。已落地：整数放大、交错换对角、1–2 命中跟格、锁满后跳过补扫、双码 **1465·60**（168.6）、四码 **1465·30 整屏同换**（168.9）。60 Hz V27 唯一上限约 169，这两条都贴上了。下面不要当回归清单自动执行。

不要做（实测否定）：四码分析改 30；60 Hz 四码 60 FPS **拿太近**（125.6）；**60 Hz 四码四格同刷**；**60 Hz 双码 1732·60**；**60 Hz 双码 60 FPS 交错一格**（约 84 KB/s）。四码 60 交错（154.4）不必再当加速项。
...
| P2 | 发送端 | 双码 1465·60 | **已落地。** 每帧 1.90，会话 **168.6 KB/s**。1732 仍只给 30 FPS |
