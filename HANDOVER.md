...
| 部件 | 版本 | 对照 |
|---|---|---|
| 网页接收端 | **v74** | 四码：一张 720 bitmap → 1 Worker、`maxSymbols: 4`。Android 30 FPS · Worker 2。相机不卡。会话 **43.3 KB/s** |
...
这是网页单码在 **60 FPS 预览** 下必须保住的数。Android 从 v73 起预览上限 30 FPS，单码还没重新测。采集掉到约 10、忙时丢弃 0，就是主线程取帧把 `requestVideoFrameCallback` 卡住了。
...
1. **网页四码** 当前对照是 v74 会话 **43.3 KB/s**（不卡、格 4、每帧 2.30）。还低于屏幕上限，是因为分析约 15 FPS。若要提速，先试 `HIGH_QUAD_INFLIGHT = 2`，不要再加每格 video 读回，不要退回主线程 `getImageData`。不要搬 APK 的 ≥3 命中重排。单码取帧不要顺手改；Android 30 FPS 之后单码还没重新测。
...
当前网页 **v74**，APK **0.8.12**。四码不卡，会话 **43.3 KB/s**，每帧 **2.30**。以 Pages 诊断第一行 `网页：v74` 为准。
