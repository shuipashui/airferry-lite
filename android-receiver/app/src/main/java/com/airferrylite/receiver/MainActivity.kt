...
class MainActivity : AppCompatActivity() {
    ...
    private fun updateUi(update: TransferUpdate) { val meta = update.meta; fileText.text = meta?.let { "${it.name} · ${formatBytes(it.size)}" } ?: "未识别文件"; val percent = if (update.total == 0) 0 else update.received * 100 / update.total; progress.progress = percent; missingText.text = "缺失片段：" + assembler.missing().take(40).joinToString(",").ifBlank { "无" }; if (update.complete != null) statusText.text = "接收完成，正在保存" else if (update.error != null) statusText.text = update.error else if (meta != null) statusText.text = "接收中：$percent%" }
